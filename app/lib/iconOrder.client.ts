...
// Client-side'da API'den ikon sıralamasını al
export async function getIconOrderClient(): Promise<IconItem[]> {
  try {
    const response = await fetch('/api/settings/icon-order');
    if (response.ok) {
      const data = await response.json();
      return data.iconOrder || DEFAULT_ICON_ORDER;
    }
...
// Firma verilerine göre ikonları sırala ve filtrele (client-side)
export function getOrderedIconsClient(firmaData: FirmaData, iconOrder: IconItem[]): IconItem[] {
    ...
    switch (iconConfig.id) {
      ...
      case 'harita':
        ...
        shouldInclude = firmaData.communication?.some(
          comm => comm.type === iconConfig.id && comm.url
        ) || false;
        break;
...
export function getIconInfoClient(iconId: string, iconOrder: IconItem[]): IconItem | null {
  return iconOrder.find(icon => icon.id === iconId) || null;
}

// Varsayılan sıralamayı al
export function getDefaultIconOrderClient(): IconItem[] {
  return DEFAULT_ICON_ORDER;
}
