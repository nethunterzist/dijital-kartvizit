...
<body>
                    ...
                    {{#each (parseBankAccounts iban.value) as |bank|}}
                        <div class="bank-card">
                            <div style="display: flex; align-items: center; margin-bottom: 15px;">
                                ...
                                {{/if}}
                                <div>
                                    <div style="font-weight: 700; font-size: 0.9rem; color: #FFEB3B;">{{bank.bank_label}}</div>
                                    <div style="color: #CCC; font-size: 0.8rem;">{{bank.account_holder}}</div>
                                </div>
                            </div>
                            {{#each bank.accounts}}
    ...
    function copyToClipboard(text, event) {
        ...
        navigator.clipboard.writeText(text).then(function() {
            ...
            btn.style.boxShadow = '0 0 15px rgba(0, 230, 118, 0.5)';
            setTimeout(() => { 
