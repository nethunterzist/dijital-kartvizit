export const purpleRainTemplate = `
<!DOCTYPE html>
<html lang="tr">
<head>
    <meta charset="UTF-8">
    ...
    <style>
        * {
        ...
        .person-name {
            ...
            margin-bottom: 6px;
            color: #E1BEE7;
            text-shadow: 1px 1px 2px rgba(156, 39, 176, 0.3);
        }
        .position {
            font-size: 1rem;
            color: #CE93D8;
        ...
        .icon-card {
            ...
            backdrop-filter: blur(10px);
            box-shadow: 0 4px 15px rgba(225, 190, 231, 0.2);
        }
        .icon-card:active {
            background: rgba(255, 255, 255, 0.2);
            transform: scale(0.95);
        ...
        <div class="custom-popup-content" style="max-width: 500px;">
            ...
            <div class="bank-accounts-list">
                {{#if iban.value}}
                    {{#each (parseBankAccounts iban.value) as |bank|}}
                        <div class="bank-card">
                            <div style="display: flex; align-items: center; margin-bottom: 15px;">
                                ...
                                {{/if}}
                                <div>
                                    <div style="font-weight: 600; font-size: 0.9rem; color: #9C27B0;">{{bank.bank_label}}</div>
                                    <div style="color: #673AB7; font-size: 0.8rem;">{{bank.account_holder}}</div>
                                </div>
                            </div>
                            {{#each bank.accounts}}
                                <div style="display: flex; align-items: center; margin-bottom: 8px;">
                                    <span style="display: inline-flex; width: 28px; height: 28px; border-radius: 50%; background: linear-gradient(135deg, #9C27B0, #673AB7); color: #FFFFFF; font-weight: 600; align-items: center; justify-content: center; margin-right: 8px; font-size: 0.75rem;">
                                        {{#if this.currency}}
                    ...
                    {{/each}}
                {{else}}
                    <div style="text-align:center; color:#673AB7; padding: 32px;">Tanımlı banka hesabı bulunamadı.</div>
                {{/if}}
            </div>
        </div>
    </div>
