...
const PhonePreview: React.FC<PhonePreviewProps> = ({
  ...
  const templateHtml = useMemo(() => {
    ...
    allIcons.push({
      ...
      label: 'QR Kod',
      url: '#',
      customClass: templateId === 1 ? 'icon-qr' : ''
    });
    ...
    communicationAccounts.forEach(acc => {
      const iconMap: { [key: string]: string } = {
        ...
        'adres': 'fas fa-map-marker-alt'
      };
      
      // Gold template için özel class ataması
      let iconClass = '';
      if (templateId === 1) {
        ...
        iconClass = classMap[acc.type] || 'icon-link';
      }
      
      allIcons.push({
        icon: iconMap[acc.type] || 'fas fa-link',
        label: acc.label || (acc.type === 'telefon' ? 'Telefon' : acc.type === 'eposta' ? 'Mail' : acc.type === 'whatsapp' ? 'WhatsApp' : acc.type === 'adres' ? 'Adres' : acc.type),
        url: '#',
    ...
    socialMediaAccounts.forEach(acc => {
      ...
      if (templateId === 1) {
        ...
        iconClass = classMap[acc.platform] || 'icon-link';
      }
    ...
    // Paylaş butonunu ekle - Gold template için
    if (templateId === 1) {
      allIcons.push({
        icon: 'fas fa-share-alt',
        label: 'Paylaş',
        url: '#',
        customClass: 'icon-paylas'
      });
    }
    ...
    allIcons.forEach(icon => {
      ...
      if (templateId === 1) {
        iconsHtml += `
          <div class="icon-item">
            <a href="${icon.url}" class="icon-link ${(icon as any).customClass || ''}">
              <i class="${icon.icon}"></i>
            </a>
    ...
    const mobileCSS = `
      <style>
        ...
        .icon-link {
          ...
          background: rgba(246, 246, 246, 0.28) !important;
        ...
        /* Gold template icon colors */
        .icon-telefon i, .fa-phone { color: #4CAF50 !important; }
        .icon-adres i, .fa-map-marker-alt { color: #FF5722 !important; }
        .icon-whatsapp i, .fa-whatsapp { color: #25D366 !important; }
        .icon-instagram i, .fa-instagram { color: #E4405F !important; }
        .icon-email i, .fa-envelope { color: #FF9800 !important; }
        .icon-paylas i, .fa-share-alt { color: #2196F3 !important; }
        .icon-qr i, .fa-qrcode { color: #9C27B0 !important; }
        .icon-website i, .fa-globe { color: #607D8B !important; }
        
        /* Tüm icon linkler için güçlü border */
        a[class*="icon-link"], .icon-item a {
                ...
                <head>
                  ...
                  <style>
                    ...
                    * {
                    ...
                    /* GOLD TEMPLATE ORİJİNAL CSS - RADİKAL ÇÖZÜM! */
                    :root {
                        --gold-primary: #D4AF37;
                        --gold-light: #F7E98E;
                        --gold-dark: #B8860B;
